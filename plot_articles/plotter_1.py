...
points += [ r"$t\bar{t}$+lf" ]
files  += [ "/home/pmandrik/work/projects/XYH/gitrepo/XYH/opendata_process/pd_file_650_MY_375_lf.root" ]

points += [ r"$t\bar{t}$+hf" ]
files  += [ "/home/pmandrik/work/projects/XYH/gitrepo/XYH/opendata_process/pd_file_650_MY_375_hf.root" ]

points += [ r"ttH" ]
files  += [ "/home/pmandrik/work/projects/XYH/gitrepo/XYH/opendata_process/pd_file_650_MY_375_ttH.root" ]
...
for point, file in zip(points, files):
  ...
  for hist_name in ["selections_nice", "N_bjets", "N_ljets", "N_l"] + ["bb_all0", "qq_all0", "qqb_all0", "nul_all", "blnu_all", "tt_all", "HY_all"] + ["bb_al_BMl", "qq_all_BM", "bqq_all_BM", "nul_all_BM", "blnu_all_BM", "tt_all_BM", "HY_all_BM", "tt_eval", "HY_eval"]:
...
def make_hists_plot( h_pt_data, label, outname, bins = np.linspace(-5, 5, 30), markers=None, cols=None, labelsx=None, labely=None, range_y=None, limitx0 = 0, limitx=None, dense = False, poitns_=False, title_=False):
  ...
  else : plt.legend(title=title_, loc='upper left')
...
if False :
    ...
    #for hist_name, axis_label in zip(["bb_all0", "qq_all0", "qqb_all0", "nul_all", "blnu_all", "tt_all", "HY_all"], ["bb_all0", "qq_all0", "qqb_all0", "nul_all", "blnu_all", "tt_all", "HY_all"]):
    reqs = [ 
      ["tt", ["tt_eval", "tt_all_BM", "tt_all"], ["DNN", r"$\chi^2$", "Combinatorics"], r"$M(t\bar{t})$ GeV", 2000],
      ["HY", ["HY_eval", "HY_all_BM", "HY_all"], ["DNN", r"$\chi^2$", "Combinatorics"],  r"$M(HY)$ GeV", 2500],
    ]

    print( answer.keys() )

    points = [ (1900, 1600) ]
    for outname, hist_names, labels, axis_label, limitx in reqs:
      data_points = []
      ...
      unfilled_markers = [m for m, func in Line2D.markers.items() if func != 'nothing' and m not in Line2D.filled_markers]
      make_hists_plot( data_points, axis_label, outname, bins = bins, cols=colors_t2, limitx=limitx, dense=True, poitns_=labels, title_ = r'$(M_X, M_Y) = ' + str(points[0]) + '$, [GeV]' )
