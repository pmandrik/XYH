  ...
  for(int i = 0; i < delphes_files.size(); i++){
    ...
    for(;entry < entrys;entry++){
      ...
      */
      ...
      myfile << TMath::Log(nu0.Pt() + 1) << " " << nu0.Phi()/3.15 << " " << TMath::Log(l.Pt() + 1) << " " << l.Eta()/2.5 << " " << l.Phi()/3.15 << " " << muon_candidates.size() << " " << electron_candidates.size() << " " << TMath::Log(Wl.Pt() + 1) << " " << Wl.Eta()/2.5 << " " << Wl.Phi()/3.15 << " " << TMath::Log(Wl.M() + 1) << " ";
      myfile << endl;
    }
    file->Close();
  }
