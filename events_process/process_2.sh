...
for cut in "lf"; do
  ...
  root -l -b -q "process_delphes.cpp(\""$file_ttbar"\",\"pd_file_1900_MY_475_"$cut".root\", \"\", \"csv_tbarx.csv\" ,\"$path_train/eval2_ttbar_MX_1900_MY_475.txt\", \""$cut"\", 475)"
  root -l -b -q "process_delphes.cpp(\""$file_ttbar"\",\"pd_file_1900_MY_1600_"$cut".root\", \"\", \"csv_tbarx.csv\" ,\"$path_train/eval2_ttbar_MX_1900_MY_1600.txt\", \""$cut"\", 1600)"
done;
exit
